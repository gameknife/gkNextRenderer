...
Options::Options(const int argc, const char* argv[])
{
    ...
    options.add_options()
        ...
        ("vulkan-driver", "Vulkan driver selection: native, lvp, or dozen (Windows).",
        ...
        ("tui-ssaa", "Supersample factor for --tui hidden rendering (1-16).", cxxopts::value<uint32_t>(TuiSsaa)->default_value("4"))
        ("tui-cell-mode", "TUI character layout: half or quadrant.", cxxopts::value<std::string>(TuiCellMode)->default_value("half"))
        ...
        ("cvar", "Apply a startup CVar override, e.g. --cvar \"r.upscaler.type 1\". Can be repeated.", cxxopts::value<std::vector<std::string>>(CVarOverrides))
