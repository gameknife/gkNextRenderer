...
{
    enum class EAuxPrimitiveType : uint32_t
    {
        Line = 0,
        Point = 1,
    };

    enum EAuxPrimitiveFlags : uint32_t
    {
        AuxPrimitiveDepthTest = 1u << 0u,
    };
    ...
    class FAuxDrawSystem final
    {
    public:
        void AddLine(glm::vec3 from, glm::vec3 to, glm::vec4 color, float size, bool depthTest);
        void AddBox(glm::vec3 min, glm::vec3 max, glm::vec4 color, float size, bool depthTest);
        void AddPoint(glm::vec3 location, glm::vec4 color, float size, int32_t durationInTick, bool depthTest);

        void ConsumeFramePrimitives(std::vector<FAuxPrimitiveGpu>& outPrimitives, uint32_t maxPrimitiveCount);
        std::vector<FAuxPrimitiveGpu> ConsumeFramePrimitives(uint32_t maxPrimitiveCount);
        uint32_t LastOverflowCount() const { return lastOverflowCount_; }

    private:
        struct FPersistentPrimitive
        {
