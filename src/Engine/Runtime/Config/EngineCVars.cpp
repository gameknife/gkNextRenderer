...
namespace NextCVar
{
    void RegisterEngineCVars(FCVarSystem& cvars, Runtime::Config::UserSettings& settings, Runtime::Config::ShowFlags& showFlags, NextEngine* engine)
    {
        ...
        GK_CVAR_FLOAT_RANGE("r.fsr.fireflySigma", settings, FSRFireflySigma, 2.5f, ECVarFlags::Archive, "Temporal upscaler post-filter isolated highlight rejection threshold in neighborhood standard deviations", 1.0, 8.0);
