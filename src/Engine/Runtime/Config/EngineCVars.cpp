...
namespace NextCVar
{
    void RegisterEngineCVars(FCVarSystem& cvars, Runtime::Config::UserSettings& settings, Runtime::Config::ShowFlags& showFlags, NextEngine* engine)
    {
        ...
        GK_CVAR_BOOL("r.fsr.postFilter", settings, FSRPostFilter, true, ECVarFlags::Archive, "Apply display-resolution bilateral noise and firefly suppression after temporal FSR");
        GK_CVAR_UINT_RANGE("r.fsr.postFilterPasses", settings, FSRPostFilterPasses, 3, ECVarFlags::Archive, "FSR post-filter a-trous pass count", 1, 4);
        GK_CVAR_FLOAT_RANGE("r.fsr.postFilterStrength", settings, FSRPostFilterStrength, 0.65f, ECVarFlags::Archive, "FSR post-filter blend strength", 0.0, 1.0);
        GK_CVAR_FLOAT_RANGE("r.fsr.postFilterLumaSigma", settings, FSRPostFilterLumaSigma, 0.10f, ECVarFlags::Archive, "FSR post-filter color/luminance edge threshold", 0.01, 0.5);
        GK_CVAR_FLOAT_RANGE("r.fsr.fireflySigma", settings, FSRFireflySigma, 2.5f, ECVarFlags::Archive, "FSR post-filter isolated highlight rejection threshold in neighborhood standard deviations", 1.0, 8.0);
        ...
        GK_CVAR_BOOL("r.dlss.jitterInvertY", settings, DLSSJitterInvertY, false, ECVarFlags::Archive, "Invert DLSS/TAA projection jitter Y for Streamline sign validation");
