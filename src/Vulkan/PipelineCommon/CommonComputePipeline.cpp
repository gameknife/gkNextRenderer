...
namespace Vulkan::PipelineCommon
{
    ...
    SoftAmbientCubeGenPipeline::SoftAmbientCubeGenPipeline(const SwapChain& swapChain, const std::vector<Assets::UniformBuffer>& uniformBuffers, const Assets::Scene& scene):swapChain_(swapChain)
    {
                // Create descriptor pool/sets.
        const auto& device = swapChain.Device();
        const std::vector<DescriptorBinding> descriptorBindings =
    ...
    SoftAmbientCubeGenPipeline::~SoftAmbientCubeGenPipeline()
    {
        if (pipeline_ != nullptr)
        {
            vkDestroyPipeline(swapChain_.Device().Handle(), pipeline_, nullptr);
    ...
    VkDescriptorSet SoftAmbientCubeGenPipeline::DescriptorSet(uint32_t index) const
    {
        return descriptorSetManager_->DescriptorSets().Handle(index);
    }
}
