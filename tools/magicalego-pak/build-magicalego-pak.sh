...
# Build assets/paks/magicalego.pak from the list in magicalego-files.txt.
...
# locally or copied in by the author.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"

FILE_LIST="${SCRIPT_DIR}/magicalego-files.txt"
OUTPUT_PAK=""
MANIFEST_PATH=""
PACK_LOG_PATH=""
PACKAGER_PATH=""
...
print_usage() {
    cat <<'EOF'
Usage: build-magicalego-pak.sh [options]

Options:
  --file-list PATH     Text file listing paths (relative to repo root) to pack
  --output-pak PATH    Output pak path (default: assets/paks/magicalego.pak)
...
if [[ -z "${OUTPUT_PAK}" ]]; then
    OUTPUT_PAK="${REPO_ROOT}/assets/paks/magicalego.pak"
fi

if [[ -z "${MANIFEST_PATH}" ]]; then
    MANIFEST_PATH="${SCRIPT_DIR}/output/magicalego_manifest.json"
fi

if [[ -z "${PACK_LOG_PATH}" ]]; then
    PACK_LOG_PATH="${SCRIPT_DIR}/output/magicalego_pack.log"
fi
...
STAGE_ROOT="${PACKAGER_RUNTIME_ROOT}/magicalego_pak_stage"

case "${STAGE_ROOT}" in
    "${PACKAGER_RUNTIME_ROOT}/magicalego_pak_stage") ;;
    *)
        echo "Refusing to delete unexpected stage directory: ${STAGE_ROOT}" >&2
        exit 1
        ;;
esac
...
PACKAGER_ARGS=(
    "--out" "${OUTPUT_PAK}"
    "--src" "magicalego_pak_stage"
    "--root" "magicalego_pak_stage"
