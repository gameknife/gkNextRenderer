...
if (-not $FileList)  { $FileList  = Join-Path $ScriptDir 'magicalego-files.txt' }
if (-not $OutputPak) { $OutputPak = Join-Path $RepoRoot 'assets\paks\magicalego.pak' }
if (-not $Manifest)  { $Manifest  = Join-Path $ScriptDir 'output\magicalego_manifest.json' }
if (-not $PackLog)   { $PackLog   = Join-Path $ScriptDir 'output\magicalego_pack.log' }
...
function Assert-SafeStageRoot($stageRoot, $runtimeRoot) {
    $stageFull = [System.IO.Path]::GetFullPath($stageRoot)
    $runtimeFull = [System.IO.Path]::GetFullPath($runtimeRoot)
    if ((Split-Path -Leaf $stageFull) -ne 'magicalego_pak_stage') {
        throw "Refusing to delete unexpected stage directory: $stageFull"
    }
    if (-not $stageFull.StartsWith($runtimeFull, [System.StringComparison]::OrdinalIgnoreCase)) {
        throw "Refusing to delete stage outside packager runtime root: $stageFull"
    }
}
...
$StageRoot = Join-Path $PackagerRuntimeRoot 'magicalego_pak_stage'
Assert-SafeStageRoot $StageRoot $PackagerRuntimeRoot
...
$packagerArgs = @('--out', $OutputPak, '--src', 'magicalego_pak_stage', '--root', 'magicalego_pak_stage', '--manifest', $Manifest)
