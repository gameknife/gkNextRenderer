...
if (-not $FileList)  { $FileList  = Join-Path $ScriptDir 'brotato3d-files.txt' }
if (-not $OutputPak) { $OutputPak = Join-Path $RepoRoot 'assets\paks\brotato3d.pak' }
if (-not $Manifest)  { $Manifest  = Join-Path $ScriptDir 'output\brotato3d_manifest.json' }
if (-not $PackLog)   { $PackLog   = Join-Path $ScriptDir 'output\brotato3d_pack.log' }
...
$StageRoot = Join-Path $PackagerRuntimeRoot 'brotato3d_pak_stage'
...
$packagerArgs = @('--out', $OutputPak, '--src', 'brotato3d_pak_stage', '--root', 'brotato3d_pak_stage', '--manifest', $Manifest)
