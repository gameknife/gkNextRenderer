...
# Build assets/paks/brotato3d.pak from the list in brotato3d-files.txt.
...
# locally (e.g. via scripts/import_brotato_placeholder.py or a manual copy).

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"

FILE_LIST="${SCRIPT_DIR}/brotato3d-files.txt"
OUTPUT_PAK=""
MANIFEST_PATH=""
PACK_LOG_PATH=""
PACKAGER_PATH=""
...
print_usage() {
    cat <<'EOF'
Usage: build-brotato3d-pak.sh [options]

Options:
  --file-list PATH     Text file listing paths (relative to repo root) to pack
  --output-pak PATH    Output pak path (default: assets/paks/brotato3d.pak)
...
if [[ -z "${OUTPUT_PAK}" ]]; then
    OUTPUT_PAK="${REPO_ROOT}/assets/paks/brotato3d.pak"
fi

if [[ -z "${MANIFEST_PATH}" ]]; then
    MANIFEST_PATH="${SCRIPT_DIR}/output/brotato3d_manifest.json"
fi

if [[ -z "${PACK_LOG_PATH}" ]]; then
    PACK_LOG_PATH="${SCRIPT_DIR}/output/brotato3d_pack.log"
fi
...
STAGE_ROOT="${PACKAGER_RUNTIME_ROOT}/brotato3d_pak_stage"
...
PACKAGER_ARGS=(
    "--out" "${OUTPUT_PAK}"
    "--src" "brotato3d_pak_stage"
    "--root" "brotato3d_pak_stage"
