...
func newScadComposeCommand(ctx appContext) *cobra.Command {
	...
	cmd.Flags().StringVarP(&outPath, "out", "o", "",
		"output .scad path (default source/generated or proc/generated by spec kind)")
	return cmd
}
...
func composeSpecFile(ctx appContext, specPath string, outPath string) error {
	...
	if outPath == "" {
		...
		outPath = filepath.Join(ctx.repoRoot, "assets", "scad", category, "generated", spec.Name+".scad")
	}
