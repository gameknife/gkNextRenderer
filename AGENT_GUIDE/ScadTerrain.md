...
- 视口相机使用右键拖动旋转、空白处左键拖动平移、滚轮缩放；右键旋转优先于可视化编辑，
  左键只有命中并拖动手柄时才修改参数。平移和缩放速度会参考场景 bounds，公里级地形无需大量滚轮。
...
- 视口采用两阶段编辑：首次点击未选中手柄只选择并定位右侧面板，再次按住已选中手柄并拖动才
  修改参数；Feature 与规则共享单一选中态，右侧只展开当前项。按住 Shift 拖动已选中项的手柄
  会先复制完整 Feature/规则，再继续编辑新副本；未超过拖动阈值时不会产生副本。
