...
- **LDraw** 
    ...
    - 从今年春节开始，我便有意的开始考察基于LLM的世界构建，尝试过Minecraft里让LLM来生成自然语言描述的结构，尝试过在MagicaLego里让LLM来生成自然语言描述的结构，感觉这是一套比较好的流程，不论Minecraft，MagicaLego，还是现在的BricksPlayer，他们的基础零件库都是可枚举，可训练的。用这些零件来“建模”，比直接让LLM生成mesh+pbrmaterial的数字资产，逻辑性强了很多。并且这些零件不只是一种”表现结构“，他还可以带逻辑，带功能。零件之间本身的连接关系，也可以带来很多的逻辑。
    - 后续BricksPlayer确实可以有一些玩法性的突破，和BrickLink Studio这种做出区别，更像是一个可以打开，随机拆装的数字乐高。实时的路径追踪渲染，让玩家可以在里面随意的摆放镜头。