...
本文以 2026-07-31 的代码为基线，定义 `NextDayz` 从 3C/搜刮演示推进到单人 PVE 生存纵切的产品边界、运行时架构和验收契约。复杂移动、ScadRig 分层动画的现行约束仍以 [复杂 3C 与 ScadRig 分层动画设计](nextdayz-3c-scadrig-design.md) 为准；本文只扩展生存、库存、刷新、僵尸和完整局内循环。
