...
> **D1（战斗深度）交付追记（2026-07，commit `c297f41d`）**：原列于此的"单位永不转向 / 无碰撞占位 / 配置硬编码"三条已被 D1 消化——朝向+炮塔（定点 `Atan2FromVec2`/`TurnToward`）、`FOccupancyGrid` 占位+软推离+建筑 footprint 写 grid、`NextRAConfig` 表化为 `FUnitDef[]` + 装甲/武器克制表，并新增 rocketeer/turret/wall 三个条目。以下为**仍然成立**的骨架/占位项：
...
- **无经济概念**：sim 无任何 per-player 状态（资金/电力），生产免费；建筑只能预置不能建造。

> **结论**：MVP+D1 是一个"战斗有深度、经济为零、网络假、确定性地基扎实"的原型。后续任何方向都能稳健地往上盖。
...
| --- | --- | --- | --- |
| 建造场 + 侧边栏建造（选建筑→计时→地图放置） | ❌ 无 | **最大缺口**，红警灵魂 | **D2 经济+建造（当前迭代 E2）** |
| 资源经济（矿车采矿→精炼厂→资金） | ❌ 无 | 生产免费，无资源概念 | **D2 经济+建造（当前迭代 E0/E1）** |
| 电力系统 | ❌ 无 | | **D2 经济+建造（当前迭代 E3）** |
| 多生产线（兵营 / 战车厂 / 机场） | ⚠️ 仅兵营 | | **D2 经济+建造（当前迭代 E4）** |
| 科技树 / 建筑前置 | ❌ 无 | | **D2 经济+建造（当前迭代 E4）** |
| 防御建筑 / 炮塔 / 围墙 | ✅ D1 已交付 | 炮塔自动索敌+炮管追踪、围墙阻挡、摧毁释放占位 | ~~D1 战斗深度~~ |
| 兵种克制（rock-paper-scissors） | ✅ D1 已交付 | 装甲/武器克制表 + rocketeer 反坦克兵 | ~~D1 战斗深度~~ |
| 单位碰撞 / 动态避让 | ✅ D1 已交付 | 占位网格 + 目标格避让 + 软推离阵型 | ~~D1 战斗深度~~ |
| 朝向 / 炮塔 / 动画 | ⚠️ 朝向/炮塔 ✅（D1），动画 ❌ | 车身/炮塔独立朝向 + 帧插值；骨骼动画未做 | D6 表现进阶 |
| 编队 / Stop / Hold / 巡逻 | ❌ 无 | 仅 Move/Attack/AttackMove/Produce（+当前迭代新增 PlaceBuilding/CancelProduce/SetRally） | D3 操作手感 |
| 迷雾 / 视野 | ❌ 无 | | D6+ 进阶玩法 |
| 多人联机 / 回放 | ⚠️ 骨架死代码 | transport/replay 零调用 | D4 联机激活 |
| 真实 AI | ⚠️ 脚本占位 | 只造步兵、无策略；当前迭代 E5 升级为"懂经济的脚本 AI" | D5 AI |
...
### D1 — 战斗与单位深度 ✅ **已交付（2026-07，commit `c297f41d`）**

- **范围**：朝向 + 炮塔追踪、装甲/武器类型表克制、整数占位 + 软推离碰撞、防御炮塔 + 围墙、Config 表化。
- **交付**：C0–C5 全部落地，消化 T1/T7/T11(facing)；确定性双跑通过。见 [`combat-depth-design.md`](combat-depth-design.md) / [`combat-depth-plan.md`](combat-depth-plan.md)（均已标"已实现"）。
- **遗留**：T9（hp clamp）计划内提及但未落地，移交 D2 迭代 E0 消化。

### D2 — 经济 + 建造系统（红警灵魂）⬅️ **已选定为当前迭代（目标：红警基础 Playable）**
...
- **工作量**：**最大**（红警最复杂的系统）。本轮按 E0–E7 八里程碑一次推进（E0 地基后 E1 资源与 E2 建造可并行），并顺带 AI 经济适配（E5）与表现补强（E6 可裁剪）。
- **关键决策点**：✅ 已定（design §1.5）——矿车**可**被攻击 / 资源**有限**枯竭不再生 / 电力**降效**（生产减速+炮塔离线）非硬禁止 / 胜负沿用基地判负。
- **状态**：📝 已出设计 + 计划，见 [`economy-build-design.md`](economy-build-design.md) / [`economy-build-plan.md`](economy-build-plan.md)。顺带消化 T3/T9（E0）、审视 T8（E7）、T5 可选（E5）。
...
| # | 技术债 | 影响 | 状态 / 归属 |
| --- | --- | --- | --- |
| **T1** | 配置硬编码（`NextRAConfig` 分散 constexpr if-else） | 加兵种/建筑改 5+ 处源码，阻碍 D2/D5 | ✅ **D1-C0 已消化**（表化为 `FUnitDef[]`） |
| **T2** | 网络栈死代码（LoopbackTransport/Replay/INetTransport 零调用，playerCount=1） | "帧同步"名实不符，阻碍 D4 | ⬜ D4 |
| **T3** | `actors_` 只增不减（[`SimWorld.cpp`](../../src/Application/Game/NextRA/Sim/SimWorld.cpp) 只 push 不 erase，死 actor 靠 `registry.valid` 过滤） | 长局性能单调退化、每系统全表扫描含死 actor | 🔨 **当前迭代 E0 消化**（DeathSystem 同步 erase） |
| **T4** | `Order.cpp`/`Replay.cpp` 序列化 `memcpy POD`（[`Order.cpp:9-26`](../../src/Application/Game/NextRA/Net/Order.cpp)） | 跨字节序/跨平台不安全，阻碍真联机 | ⬜ D4（当前迭代新增 3 种 order 时沿用现约定，D4 统一换） |
| **T5** | `SimRandom.NextU32()` 从未被调用 | 确定性是"无随机"而非"确定性随机"，设施摆设 | ⬜ 当前迭代 E5 可选（AI 抖动）；否则留给命中率/弹道散布 |
...
| **T7** | `acquireRange` 兵种无关硬编码 | 兵种索敌行为无差异 | ✅ **D1-C0 已消化**（读 `UnitDef.acquireRange`） |
| **T8** | `Step` 单 tick 跑两次 combat pass（[`SimWorld.cpp:97`](../../src/Application/Game/NextRA/Sim/SimWorld.cpp) Step 内两遍 Targeting+Combat） | 隐含"DPS 翻倍"规则，调参易踩坑 | 🔨 **当前迭代 E7 审视**（保留改数值 or 统一单 pass，结论入 journal） |
| **T9** | `CombatSystem` 扣血无 clamp（hp 可为负，[`SimWorld.cpp:603`](../../src/Application/Game/NextRA/Sim/SimWorld.cpp)） | 功能无害但 SyncHash 混入负值，跨实现需一致 | 🔨 **当前迭代 E0 消化**（D1 计划内提及但未落地） |
| **T10** | "Net delay/Drop/Reorder" 滑块只影响 AI（与 `LoopbackTransport` 名义功能脱节） | UI 误导 | ⬜ D4 |
| **T11** | 遗留未启用字段（`FMobile.pingPong/pointA/pointB`、`facing`） | 代码噪音 | `facing` ✅ D1-C1 已启用；pingPong → D3 Patrol 可复用或删除 |

> **建议**：任何迭代接手时，先扫本表，把"顺手能修且影响本方向"的债纳入该迭代范围。当前迭代（D2 经济+建造）认领 T3/T9（E0）、T8（E7）、T5（E5 可选）。
...
| [`combat-depth-design.md`](combat-depth-design.md) | D1 战斗深度 架构设计 | ✅ 已交付 |
| [`combat-depth-plan.md`](combat-depth-plan.md) | D1 战斗深度 开发计划 | ✅ 已交付 |
| [`economy-build-design.md`](economy-build-design.md) | D2 经济+建造（红警基础 Playable）架构设计 | 📝 待实现（当前迭代） |
| [`economy-build-plan.md`](economy-build-plan.md) | D2 经济+建造 开发计划（E0–E7） | 📝 待实现（当前迭代） |
