...
| generated | `assets/scad/gen/*.scad` | `gnb scad compose` 的派生产物，会被覆盖 |
...
./gnb.sh shot --scene assets/scad/gen/deadly_roadtrip_map.scad
```
...
- `gen/*.scad` 文件头记录源 spec 与内容 hash。修改生成文件不会反向更新 spec，下次 compose 会覆盖修改。
