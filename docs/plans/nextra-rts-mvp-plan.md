...
任务：
- [ ] 在 [`src/CMakeLists.txt`](../../src/CMakeLists.txt) 加 `add_executable(NextRA ...)`（参照 `CharacterDemo` `:206`、链接 `:602` 段），`target_include_directories` + `DEV_MODE=1`，链接 `gkNextEngine`。
- [ ] `NextRAGameInstance.{hpp,cpp}` 继承 `NextGameInstanceBase`（[`GameInstance.hpp:20`](../../src/Engine/Runtime/GameInstance.hpp)），实现 `OnInit/OnTick/OnDestroy/OnRenderUI` 空壳 + `CreateGameInstance`（参照 [`CharacterDemoGameInstance.cpp:32`](../../src/Application/Game/CharacterDemo/CharacterDemoGameInstance.cpp)）。
- [ ] `BeforeSceneRebuild`（[`GameInstance.hpp:36`](../../src/Engine/Runtime/GameInstance.hpp)）注入：大平面地形（`FProcModel::CreateBox` 扁盒，[`FProcModel.h:11`](../../src/Engine/Assets/Loaders/FProcModel.h)）+ 几个彩色盒（`SceneBuilder::AddLambertianMaterial`/`CreateRenderNode`，[`SceneBuilder.h:12/17`](../../src/Engine/Runtime/Scene/SceneBuilder.h)）。
- [ ] `RtsCamera` + `OverrideRenderCamera`：固定俯角，WASD / 边缘平移、滚轮缩放。
...
任务：
- [ ] `Sim/Fixed.h`：`fixed` 类型 + 四则 / 比较 / `FromInt/ToInt/ToFloat` + `Sqrt`（设计 §5.1）。
- [ ] `Sim/WMath.h`：`WPos/WDist/WAngle/WVec/CPos` + 互转 + 定点 `Sin/Cos` 查表（§5.2/5.3）。
- [ ] `Sim/SimComponents.h`：`FSimTransform/FOwner/FHealth/FMobile/...`（§5.6，先用到的几个）。
- [ ] `Sim/SimWorld.{h,cpp}`：独立 `entt::registry` + `Step(tick)` + 系统编排骨架（先只有 Movement）+ 稳定 actor 列表（§5.6 遍历顺序）。
- [ ] `NextRAGameInstance::OnTick` 接入固定 tick 累加器（`SIM_HZ=20`，§5.4），算 `renderAlpha`。
- [ ] `Render/RenderProxySystem`：sim actor ↔ 渲染 node 映射（`FRenderLink`），每帧 `lerp(prevPos,currPos,alpha)` 写 `Node::SetTranslation`（[`Node.h:25`](../../src/Engine/Assets/Core/Node.h)）。
- [ ] 把 `Fixed/WMath` 纯逻辑加入 `gkNextUnitTests` 源集，写定点 / 坐标转换单测。
...
任务：
- [ ] `Net/Order.h`：`FOrder` + 序列化往返（§6.1）。
- [ ] `Net/OrderManager`（单机最小版）：收集本地 order，`GetExecOrders(tick)` 直接返回本 tick（latency=0）。
- [ ] 输入：左键单击选 / 拖框多选（`RayCastGPU` [`Engine.hpp:156`](../../src/Engine/Runtime/Engine.hpp) 或 CPU `RayCastInCPU` [`CPUAccelerationStructure.h:127`](../../src/Engine/Assets/Acceleration/CPUAccelerationStructure.h)）；右键地面 → `Move` order（命中点 float→`WPos`，§8）。
- [ ] `Sim/PathfindGrid.{h,cpp}`：定点格子 A\*（确定性 tie-break，§5.5）。
- [ ] `Systems/OrderApplySystem` + `Systems/MovementSystem`：order 落 goal → A\* 路径 → `FMobile` 逐 tick 沿 waypoint 定点推进。
- [ ] 选择表现：选中圈（`Render/NextRAHud` 起步）。
...
任务：
- [ ] 组件补全 `FHealth/FAttack`（§5.6）；第二兵种（坦克：盒 + 炮管，炮管随 facing）。
- [ ] `Systems/TargetingSystem`：射程 / 视野内选目标（确定性：距离 + entity id tie-break，§5.6）。
- [ ] `Systems/CombatSystem`：冷却到点扣血（瞬时命中，子弹可省略或画小球）。
- [ ] `Systems/DeathSystem`：hp≤0 销毁 actor + 移除 render node + 释放格子占位。
- [ ] 命令扩展：右键命中敌 → `Attack`；修饰键 → `AttackMove`（移动途中自动交战）。
- [ ] 血条表现（`NextRAHud`）。
...
任务：
- [ ] `FProduction` + `Systems/ProductionSystem`：兵营选中 → HUD 出"造步兵 / 坦克"按钮 → `Produce` order → 计时 → rally 点 spawn。
- [ ] 基地 `FBaseTag` + 高血量；`DeathSystem` 检测基地死亡 → 置胜负标志。
- [ ] 2 席位：player 0 / player 1 各一基地 + 兵营 + 初始单位；faction 上色区分。
- [ ] 胜负 UI：一方基地被摧毁 → 显示 Victory/Defeat。
- [ ] （可选）极简脚本 AI 占位：player 1 周期性造兵 + 攻击移动向敌基地（**走 order 通道**，保持确定性）。
...
任务：
- [ ] `Sim/SimRandom.h`：确定性 PRNG，所有 sim 随机改走它（§6.5）；接入需要随机的系统（如 tie-break 外的散布）。
- [ ] `Sim/SyncHash.{h,cpp}` + `SyncHashSystem`：每 tick 按 entity id 顺序滚 hash（pos/facing/hp/target/cooldown + PRNG state，§6.5）。
- [ ] `Net/INetTransport.h` + `Net/LoopbackTransport`：进程内多 World 投递，含**人工延迟 + 丢包 / 乱序注入**开关（§6.4）。
- [ ] `OrderManager` 升级为 lockstep：`execTick = tick + orderLatency`，gate 收齐所有 player 才 `CanAdvance`，空 tick 发心跳包（§6.2/6.3）；`OnTick` 累加器接 `CanAdvance` 限流（§5.4）。
- [ ] `Net/Replay.{h,cpp}`：录制 `{seed, latency, 每 execTick orders}` 到 `.nrarep`；回放重建 World 重算 hash。
- [ ] 测试装置：双 `SimWorld` + `LoopbackTransport` 跑同一对局；确定性双跑断言；replay 回归。
...
任务：
- [ ] HUD 完整化：选择圈 / 血条 / 生产按钮 / 当前选中信息 / 胜负横幅。
- [ ] 调试覆盖层：当前 tick、各 peer hash、order log 滚动、A\* 路径 / 格子可走性可视化（参照 CharacterDemo 的 AI/NavGrid 调试 UI 风格，[`AGENT_GUIDE/CharacterDemo.md`](../../AGENT_GUIDE/CharacterDemo.md)）。
- [ ] 运行时开关：order latency、人工延迟、丢包率（用于现场演示 lockstep 韧性）。
- [ ] （可选）minimap：俯视缩略 + 单位点。
- [ ] `docs/projects/nextra/`（可选）写一篇"NextRA 开发者指南"，并把状态从"草案"推进。
