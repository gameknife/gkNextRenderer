...
> **方向更新（2026-07-15）：** 本计划中“统一 provider/profile/bridge”的基础设施方向继续保留；“通用 Editor Agent、repo tools、开放式 Agent Runtime”等后续目标不再执行。AgentDriver 与 LLM 无关，其后续演进应由验证系统独立规划，不再捆绑在 AI 重构中。当前边界见 [NextAI 面向具体产品能力的目标架构](../designs/nextai-product-focused-architecture.md)，执行顺序见 [轻量化重构计划](nextai-product-focused-refactor-plan.md)。本文仅保留为历史设计记录。
