...
///
/// The data is parallel arrays rather than an array of enemy objects. At this size either would
/// do; the habit is worth having because the loops below then walk contiguous memory and never
/// chase a reference.
/// </remarks>
internal sealed class EnemySwarm(int capacity, float radius)
{
    ...
    public int ConsumeContacts(float playerX, float playerZ, float contactDistance)
    {
        ...
        for (int i = 0; i < capacity; i++)
        {
            if (!alive[i] || !Mathx.OverlapsXZ(positionX[i], positionZ[i], playerX, playerZ, contactDistance))
            {
                continue;
            }
            alive[i] = false;
    ...
    public void SyncNodes()
    {
        for (int i = 0; i < capacity; i++)
        {
            if (alive[i])
            {
                Scene.SetNodeTranslation(nodes[i], new Vector3(positionX[i], radius, positionZ[i]));
            }
