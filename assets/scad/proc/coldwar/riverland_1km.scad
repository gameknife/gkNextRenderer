...
TERR = ["gkterr1", [1000, 1000], [176, 176], 7, [0, 2.6, 0.5], undef, "temperate",
    [
        ...
        ["road", [[-500, -140], [-390, -110], [-300, -150], [-210, -110], [-120, -150], [-30, -158], [60, -150], [150, -110], [240, -140], [330, -100], [420, -130], [500, -115]], 8],
        ["road", [[150, -110], [185, -10], [240, 80], [292, 150]], 13.6944233],
        ["road", [[240, -140], [240, -216]], 7],
        ["road", [[60, -150], [75, -240], [95, -308]], 6],
        ["road", [[-390, -110], [-380, -20], [-350, 60], [-345, 100]], 5],
        ["pad", [-300, -174], [46, 32], 0],
        ["pad", [-30, -177], [30, 18], 0],
        ["pad", [50, -176], [16, 12], 0],
        ["pad", [340, 165], [90, 64], 0],
...
ter_place(TERR, -335, -140, dz = 0)
    rotate([0, 0, -24]) cw_prop_sign_town(seed = 1);
ter_place(TERR, -270, -158, dz = 0)
    cw_nature_tree_dead(s = 1.6, seed = 2);

// ================= POI 2：桥西加油站（pad -30,-177；公路在北） =================
ter_place(TERR, -30, -177, dz = 0)
    {
...
// 桥头封锁线（路面上）
ter_place(TERR, -2, -158, dz = 0)
    rotate([0, 0, 95]) cw_prop_barrier_gate(seed = 0);
ter_place(TERR, 0, -155, dz = 0)
    rotate([0, 0, 40]) cw_prop_hedgehog();
ter_place(TERR, 36, -152, dz = 0)
    rotate([0, 0, -15]) cw_prop_hedgehog();

// ================= POI 4：军事基地（pad 340,165；大门朝西接支路） =================
ter_place(TERR, 340, 165, dz = 0)
    {
    // 周界铁丝网（西边留门洞）+ 四角岗塔
...
ter_place(TERR, 160, -100, dz = 0)
    cw_prop_sign_road(seed = 0);

// ================= POI 5：东南小镇（pad 240,-246；支路从北接入） =================
ter_place(TERR, 240, -246, dz = 0)
    {
    // 内部街道（南北主街 + 东西横街）
...
ter_place(TERR, 232, -150, dz = 0)
    cw_prop_sign_road(seed = 1);

// ================= POI 6：河畔工厂（pad 95,-332；支路从北接入） =================
ter_place(TERR, 95, -332, dz = 0)
    {
...
ter_place(TERR, -128, 228, dz = 0)
    cw_prop_campfire(seed = 4);
ter_place(TERR, -126, 226, dz = 0)
    cw_item_bedroll(seed = 2);

// ================= 湖畔营地（西南湖东岸，无 pad 贴地） =================
ter_place(TERR, -266, -318, dz = 0)
    rotate([0, 0, 250]) cw_prop_tent_military(seed = 3);
ter_place(TERR, -262, -326, dz = 0)
    cw_prop_campfire(seed = 2);
ter_place(TERR, -264, -330, dz = 0)
    cw_item_bedroll(seed = 4);
ter_place(TERR, -260, -322, dz = 0)
    cw_item_can(seed = 7);
ter_place(TERR, -268, -312, dz = 0)
    cw_item_lantern(seed = 1);
ter_place(TERR, -258, -334, dz = 0)
    cw_wpn_shotgun(seed = 2);
// 湖岸芦苇圈
ter_place(TERR, -273, -330, dz = 0)
    cw_nature_reeds(seed = 1);
ter_place(TERR, -286, -293, dz = 0)
    cw_nature_reeds(seed = 2);
ter_place(TERR, -320, -274, dz = 0)
    cw_nature_reeds(seed = 3);
ter_place(TERR, -374, -293, dz = 0)
    cw_nature_reeds(seed = 4);
ter_place(TERR, -384, -350, dz = 0)
    cw_nature_reeds(seed = 5);
ter_place(TERR, -320, -386, dz = 0)
    cw_nature_reeds(seed = 6);
ter_place(TERR, -286, -367, dz = 0)
    cw_nature_reeds(seed = 7);

// ================= 公路道具 =================
// 电力杆沿主公路北侧
ter_along(TERR, [[-500, -140], [-390, -110], [-300, -150], [-210, -110], [-120, -150], [-30, -158]], step = 85, seed = 5, offset = 0, dz = 0)
    translate([0, 5.5, 0]) cw_prop_pole_concrete(seed = $seed);
ter_along(TERR, [[60, -150], [150, -110], [240, -140], [330, -100], [420, -130], [500, -115]], step = 85, seed = 6, offset = 0, dz = 0)
    translate([0, 5.5, 0]) cw_prop_pole_concrete(seed = $seed);
// 路面弃车与路牌
ter_place(TERR, -210, -110, dz = 0)
    rotate([0, 0, 204]) cw_veh_lada(seed = 5);
ter_place(TERR, -120, -150, dz = 0)
    rotate([0, 0, -22]) cw_veh_wreck(seed = 4);
ter_place(TERR, 330, -100, dz = 0)
    rotate([0, 0, 12]) cw_veh_truck_canvas(seed = 2);
ter_place(TERR, 100, -138, dz = 0)
    rotate([0, 0, 160]) cw_veh_uaz_van(seed = 3);
ter_place(TERR, -444, -128, dz = 0)
    rotate([0, 0, -14]) cw_veh_bus(seed = 2);
ter_place(TERR, 100, -160, dz = 0)
    rotate([0, 0, 180]) cw_prop_billboard(seed = 1);
ter_place(TERR, 152, -102, dz = 0)
    rotate([0, 0, 65]) cw_prop_sign_town(seed = 2);
ter_place(TERR, 66, -242, dz = 0)
    rotate([0, 0, 80]) cw_prop_sign_road(seed = 2);

// ================= 植被生态（ter_scatter 过滤散布） =================
// 针叶/白桦混交林：缓坡草地，避水
ter_scatter(TERR, 101, 920, [-490, -490, 490, 490], [0.5, 40, 26, 3, ["grass", "grass_dark"]], rot = true, dz = 0, variants = 12, scale = [1.5, 2.9])
    lay_pick(i = $variant)
    {
        cw_nature_pine(s = 1, seed = $variant_seed);
        cw_nature_pine(s = 1, seed = $variant_seed + 1);
        cw_nature_birch(s = 1, seed = $variant_seed);
        cw_nature_pine(s = 1, seed = $variant_seed + 2);
    }
// 北坡密林带
ter_scatter(TERR, 103, 704, [-490, 120, 490, 460], [2, 42, 28, 2, ["grass", "grass_dark"]], rot = true, dz = 0, variants = 2, scale = [1.6, 2.8])
    lay_pick(i = $variant)
    {
        cw_nature_pine(s = 1, seed = $variant_seed);
        cw_nature_birch(s = 1, seed = $variant_seed);
...
// 高地岩块（随机朝向）
ter_scatter(TERR, 107, 50, [-490, -490, 490, 490], [1, 60, 60, 0, ["rock", "rock_high"]], rot = true, dz = 0, variants = 0, scale = [1, 1])
    cw_nature_rock(s = lay_randr($seed, 4, 1.4, 3.0), seed = $seed);
// 枯草带灌木与枯树
ter_scatter(TERR, 109, 55, [-490, -490, 490, 490], [1, 45, 30, 1, ["dry_grass"]], rot = true, dz = 0, variants = 0, scale = [1, 1])
    lay_pick($seed)
    {
...
// 河谷两岸疏林（让河道有掩体接近路线）
ter_scatter(TERR, 115, 80, [-150, -400, 120, 300], [0.5, 30, 24, 4, ["grass", "grass_dark"]], rot = true, dz = 0, variants = 0, scale = [1, 1])
    lay_pick($seed)
    {
...
// 低地灌木丛
ter_scatter(TERR, 113, 100, [-490, -490, 490, 490], [0.3, 25, 22, 2, ["grass", "grass_dark"]], rot = true, dz = 0, variants = 8, scale = [1, 1])
    lay_pick($seed)
    {
